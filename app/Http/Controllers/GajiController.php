...
class GajiController extends Controller
{
    ...
    public function gajiExportTemplate(Request $request)
    {
        ...
        $sheet
            ...
            ->setCellValue('E1', 'RP E')
            ->setCellValue('F1', 'RP M')
            ->setCellValue('G1', 'RP SP')
            ->setCellValue('H1', 'BULANAN');
        $pos = 2;
        $posisi = Posisi::all();
        foreach ($posisi as $l) {
        ...
        foreach ($gaji as $k) {
            ...
            $sheet->setCellValue('E' . $kolom, $k->rp_e);
            $sheet->setCellValue('F' . $kolom, $k->rp_m);
            $sheet->setCellValue('G' . $kolom, $k->rp_sp);
            $sheet->setCellValue('H' . $kolom, $k->g_bulanan);
