<form action="<?= route('saveEditJurnal') ?>" method="POST">
    @csrf
    <div class="modal-content">
        <div class="modal-header bg-costume">
            <h4 class="modal-title">Edit Jurnal</h4>
            <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <div class="modal-body">
            <?php if ($aktiva_benda->kredit == '0') : ?>
                <div class="row">
                    <div class="col-sm-3 col-md-3">
                        <div class="form-group">
                            <label for="list_kategori">Tanggal</label>
                            <input class="form-control" type="date" name="tgl" value="<?= $debit->tgl ?>" required>
                            <input class="form-control" type="hidden" name="id_jurnal_debit" value="<?= $debit->id_jurnal ?>">
                        </div>
                    </div>
                    <input type="hidden" name="id_jurnal_kredit" value="<?= $kredit->id_jurnal ?>">

                    <div class="mt-3 ml-1">
                        <p class="mt-4 ml-2 text-warning"><strong>Db</strong></p>
                    </div>
                    <div class="col-sm-4 col-md-4">
                        <div class="form-group">
                            <label for="list_kategori">Akun</label>
                            <select name="id_akun_debit" id="id_akun" class="form-control select" required="">
                                <?php foreach ($akun as $a) : ?>
                                    <?php if ($debit->id_akun == $a->id_akun) : ?>
                                        <option value="<?= $a->id_akun ?>" selected><?= $a->nm_akun ?></option>
                                    <?php else : ?>
                                        <option value="<?= $a->id_akun ?>"><?= $a->nm_akun ?></option>
                                    <?php endif ?>

                                <?php endforeach ?>
                            </select>
                        </div>
                    </div>

                    <div class="col-sm-2 col-md-2">
                        <div class="form-group">
                            <label for="list_kategori">Debit</label>
                            <input type="number" class="form-control total_edit" id="total" name="total" value="<?= $kredit->kredit ?>" readonly>
                        </div>
                    </div>
                    <div class="col-sm-2 col-md-2">
                        <div class="form-group">
                            <label for="list_kategori">Kredit</label>
                            <input type="number" class="form-control" readonly>
                        </div>
                    </div>

                    <div class="col-sm-3 col-md-3">

                    </div>

                    <div class="mt-1">
                        <p class="mt-1 ml-3 text-warning"><strong>Cr</strong></p>
                    </div>

                    <div class="col-sm-4 col-md-4">
                        <div class="form-group">
                            <select name="id_akun_kredit" id="metode" class="form-control select" required>';
                                <?php foreach ($akun as $a) : ?>
                                    <?php if ($kredit->id_akun == $a->id_akun) : ?>
                                        <option value="<?= $a->id_akun ?>" selected><?= $a->nm_akun ?></option>
                                    <?php else : ?>
                                        <option value="<?= $a->id_akun ?>"><?= $a->nm_akun ?></option>
                                    <?php endif ?>
                                <?php endforeach ?>
                            </select>
                        </div>
                    </div>

                    <div class="col-sm-2 col-md-2">
                        <div class="form-group">
                            <input type="number" class="form-control" readonly>
                        </div>
                    </div>
                    <div class="col-sm-2 col-md-2">
                        <div class="form-group">
                            <input type="number" name="kredit" class="form-control total_edit" value="<?= $kredit->kredit ?>" readonly>
                            <input class="form-control" type="hidden" name="id_jurnal_kredit" value="<?= $kredit->id_jurnal ?>">
                        </div>
                    </div>

                    <div class="col-sm-3 col-md-3">

                    </div>
                </div>
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="list_kategori">No id</label>
                            <input type="text" name="no_id" class="form-control input_detail input_aktiva" value="<?= $debit->no_urutan ?>" placeholder="No id">

                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="list_kategori">Post Center</label>
                            <select name="id_post" id="id_post" class="form-control select input_detail input_aktiva">
                                <?php if ($kredit->id_post == "0") : ?>
                                <?php else : ?>
                                @if (empty($post))
                                <?php foreach ($post_center_aktiva as $p) : ?>
                                    <option value="<?= $p->id_post ?>"><?= $p->nm_post ?></option>
                                <?php endforeach ?>
                                @else
                                <option value="<?= $kredit->id_post ?>"><?= $post->nm_post ?></option>
                                @endif
                                <?php endif ?>

                            </select>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="list_kategori">Keterangan</label>
                            <input type="text" class="form-control" name="ket" value="<?= $debit->ket ?>">
                        </div>
                    </div>

                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="list_kategori">Satuan</label>
                            <select name="id_satuan" class="form-control select2bs4 satuan input_detail input_aktiva" required>
                                <?php foreach ($satuan as $p) : ?>
                                    <?php if ($debit->id_satuan == $p->id) : ?>
                                        <option value="<?= $p->id ?>" selected><?= $p->n ?></option>
                                    <?php else : ?>
                                        <option value="<?= $p->id ?>"><?= $p->n ?></option>
                                    <?php endif ?>

                                <?php endforeach; ?>
                            </select>

                        </div>
                    </div>
                    <div class="col-md-1">
                        <div class="form-group">
                            <label for="list_kategori">Qty</label>
                            <input type="text" class="form-control input_detail input_aktiva qty_aktiva qty_monitoring1" name="qty" id="txt3" onkeyup="sum();" value="<?= $debit->qty ?>" required>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="list_kategori">Rp/Satuan</label>
                            <input type="text" name="rp_satuan" class="form-control ttlp total_penyesuaian rp_satuan_aktiva  input_detail input_aktiva " id="ttlp" name="rp_satuan" value="<?= $debit->debit ?>" onkeyup="sum();" required>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="list_kategori">PPN</label>
                            <input type="text" class="form-control input_detail total_rp_new1 input_aktiva total_rp_new" name="ppn" id="txt2" onkeyup="sum();">
                        </div>
                    </div>


                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="list_kategori">Rp + Pajak</label>
                            <input type="text" class="form-control total_penyesuaian  input_detail input_aktiva pajak_aktiva" id="total2" name="ttl_rp" required>
                        </div>
                    </div>
                </div>
                <hr>
                <div class="row justify-content-center">
                    <input type="hidden" name="id_aktiva" value="<?= $aktiva_benda->id_aktiva ?>">
                    <div class="col-lg-12">
                        <table class="table table-bordered table-sm" width="100%">
                            <thead style="text-align: center;">
                                <tr>
                                    <th></th>
                                    <th width="15%">Nama Kelompok</th>
                                    <th width="15%">Umur</th>
                                    <th width="70%">Barang <?= $aktiva_benda->id_kelompok ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($aktiva as $a) : ?>

                                    <tr>
                                        <td>
                                            <?php if ($aktiva_benda->id_kelompok == $a->id_kelompok) : ?>
                                                <input type="radio" name="id_kelompok" id="" value="<?= $a->id_kelompok ?>" checked>
                                            <?php else : ?>
                                                <input type="radio" name="id_kelompok" id="" value="<?= $a->id_kelompok ?>">
                                            <?php endif ?>

                                        </td>
                                        <td><?= $a->nm_kelompok ?></td>
                                        <td><?= $a->umur ?> Tahun</td>
                                        <td><?= $a->barang_kelompok ?></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                        </table>
                    </div>
                </div>
            <?php else : ?>
                <div class="row justify-content-center">
                    <div>
                        <h5>jurnal aktiva tidak bisa di edit karena sudah pernah disesuaikan </h5>
                    </div>
                </div>
            <?php endif ?>
        </div>
        <div class="modal-footer">
            <?php if ($aktiva_benda->kredit == '0') : ?>
                <button type="submit" class="btn btn-info">Save/Edit</button>
            <?php else : ?>
            <?php endif ?>
            <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        </div>
    </div>
</form>